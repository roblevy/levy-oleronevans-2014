...
From the elements of $\boldsymbol{Z}^\dagger$ and $\boldsymbol{Z}^*$, each sector has a complete `recipe' for making its output, in terms of the quantities of each product used as input, both domestic and imported. For the remainder of this section, the country superscript will be excluded whenever its presence is clear from the context.
...
The amount of product $r$ required by sector $s$ to produce a single unit of output is thus:
...
If the total final demand for both domestic \textit{and} imported goods is defined as $\boldsymbol{f} = \boldsymbol{f}^{\dagger} + \boldsymbol{f}^{*}$ and similarly for total investments, $\boldsymbol{n} = \boldsymbol{n}^{\dagger} + \boldsymbol{n}^{*}$, then, following equation \eqref{eq:adagger}, the technical coefficients can be calculated as:
\begin{equation}
...
\end{align}
Another benefit of the import ratios assumption is that final demand and investment have only $s$ elements to be specified, rather than the $2s$ elements shown in equation \eqref{eqn:T}.
...
\begin{enumerate}
\item Final demand, $f_s^{(i)}$: the quantity of a product consumed by the public and the government of a particular country.
\item Import propensities, $p_s^{(i,j)}$: the proportion of country $j$'s total of import of product $s$ which comes from country $i$.
\item Import ratios, $d_s^{(i)}$: the proportion of country $i$'s total demand for product $s$ which is supplied by imports.
\item Technical coefficients, $a_{r,s}^{(i)}$: the quantity of product $r$ required in country $i$ to make a single unit of product $s$.
\end{enumerate}
