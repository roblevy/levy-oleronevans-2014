...
\usepackage[backend=biber,url=false,doi=false,isbn=false,style=authoryear]{biblatex}
...
\markright{GLOBAL TRADE MODEL BASED ON LINKED IO MODELS}
...
This is perhaps a surprising result, as we might have expected the US to be the world's most significant economy.
...
\end{equation}
Figure \ref{fig:countries_domestic_vs_foreign} shows how the average response across all sectors divides between domestic response, $\eta_s^{\dagger}$, and foreign response\footnote{It should be noted here that the `rest of the world' (RoW) is not included in this analysis. Thus ``foreign'' here refers only to the countries explicitly modelled. According to \textcite{streicher_whither_2012}, the 40 WIOD countries cover 85\% of the global economy, so the effect of excluding RoW should not be too large.}, $\eta_s^{*}$, for each country. 
