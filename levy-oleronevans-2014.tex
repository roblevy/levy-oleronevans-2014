...
INSERT SELLING SENTENCE - ACTUALLY THE ABSTRACT NEEDS A GOOD REWRITE
\end{abstract}
...
At this stage, the focus is on creating a model of global trade, which will form a skeleton to which additional processes will be added in future work.
...
Such alterations would be more straightforward to perform and more intuitively comprehensible in this model than in an unwieldy whole-world input output model, in which changing any individual process would require consideration of the entire system and would likely necessitate a completely new approach to finding solutions.
...
This reformulation demonstrates the connection between the model described in this paper and the established whole-world input-output model approach, allowing us to visualise the individual country-sector to country-sector flows (or changes in such flows) for any model scenario, such as that calculated in section \ref{ss_simple_modelling_approaches} in response to a reduction of demand in China for the Vehicles sector by \$1M.
