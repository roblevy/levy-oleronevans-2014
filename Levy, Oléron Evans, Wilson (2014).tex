...
Recall from section \ref{sec:importratios} above, that goods arriving at the shores of an importing country are put into a warehouse along with domestically produced goods at which point the two become indistinguishable. Under the additional assumption that domestic sectors take from this warehouse by method of a random sample, the fraction of goods in each sample from abroad will be constant, as will the fraction of imported goods from each exporter. These fractions will be set by the import ratios and import propensities respectively. 
